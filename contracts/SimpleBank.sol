...
contract SimpleBank {
    ...
    /* Check sender enrolled  */
    modifier isEnrolled {
        require(
            enrolled[msg.sender],
            "Only enrolled sender can call this function."
        );
        _;
    }

    modifier isBalanceSufficient(uint256 amount) {
        require(
            balances[msg.sender] >= amount,
            "Only allow when balance is sufficient."
        );
        _;
    }
    ...
    function getBalance() public view isEnrolled returns (uint256) {
    ...
    function withdraw(uint256 withdrawAmount)
        public
        isBalanceSufficient(withdrawAmount)
        returns (uint256)
    {
